...
public class TouchThenRise : MonoBehaviour {
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        if(!isTriggered)
        {
            ...
            {
                gameObject.GetComponent<Rigidbody2D>().linearVelocity = new Vector2(0f, risingStrength);
                Rigidbody2D collisonRB = collision.gameObject.GetComponent<Rigidbody2D>();
                collisonRB.linearVelocity = new Vector2(0f, risingStrength + collisonRB.gravityScale);
